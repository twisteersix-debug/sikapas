...
//    /* ... konten halaman ... */
//    require_once 'includes/layout_close.php';
//
//  KEY $activeMenu:
...
function arrowOpen(array $keys): string {
    global $activeMenu;
...
.nav-item.active{background:var(--blue);color:#fff;font-weight:600;
  box-shadow:0 3px 10px rgba(30,111,191,.4)}
...
.user-caret{width:13px;height:13px;stroke:var(--gray-400);fill:none;stroke-width:2.5;transition:transform .2s;flex-shrink:0}
.user-pill.open .user-caret{transform:rotate(180deg)}

/* User dropdown */
.user-dropdown{position:absolute;top:calc(100%+8px);right:0;background:#fff;border-radius:12px;min-width:210px;box-shadow:var(--shadow-md);border:1px solid var(--gray-200);overflow:hidden;display:none;z-index:100;animation:dropIn .16s ease}
...
.dd-item{display:flex;align-items:center;gap:9px;padding:10px 15px;font-size:12.5px;color:var(--gray-800);text-decoration:none;transition:background .14s;cursor:pointer;border:none;background:none;width:100%;font-family:inherit}
...
<aside class="sidebar" id="sidebar">
  ...
  <div class="nav-item <?= subActive(['pegawai','pegawai_satker','pengingat']) ?>"
    ...
    Pegawai
    <svg class="nav-arrow <?= arrowOpen(['pegawai','pegawai_satker','pengingat']) ?>" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg>
  </div>
  ...
  <div class="nav-item <?= subActive(['dokumen','arsip']) ?>"
    ...
    Arsip Kepegawaian
    <svg class="nav-arrow <?= arrowOpen(['dokumen','arsip']) ?>" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg>
  </div>
  ...
  <div class="nav-item <?= subActive(['kgb','slks','tunjangan']) ?>"
    ...
    Laporan
    <svg class="nav-arrow <?= arrowOpen(['kgb','slks','tunjangan']) ?>" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg>
  </div>
