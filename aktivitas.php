...
$logs  = $stmt->fetchAll();
$moduls= $db->query("SELECT DISTINCT modul FROM activity_log ORDER BY modul")->fetchAll(PDO::FETCH_COLUMN);
    ...
    <table class="sikapas-tbl">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <?php
            $aksiColor = ['Tambah'=>'badge-green','Update'=>'badge-blue','Hapus'=>'badge-red','Login'=>'badge-purple','Upload'=>'badge-orange'];
          ...
          <td style="font-size:12px;color:var(--gray-600);max-width:300px"><?= htmlspecialchars($l['detail'] ?? '-') ?></td>
        </tr>
        <?php endforeach; ?>
        <?php else: ?>
        <tr><td colspan="6" class="loading">Tidak ada data aktivitas</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
