...
if ($module === 'pegawai') {
    if ($action === 'list') {
        $q = '%' . ($_GET['q'] ?? '') . '%';
        $stmt = $db->prepare("SELECT p.*, s.nama AS satker FROM pegawai p LEFT JOIN satker s ON s.id=p.satker_id WHERE p.nama LIKE ? OR p.nip LIKE ? ORDER BY p.nama LIMIT 100");
        $stmt->execute([$q, $q]);
    ...
    if ($action === 'save') {
        ...
        if ($id) {
            $stmt = $db->prepare("UPDATE pegawai SET nip=?,nama=?,jabatan=?,golongan=?,satker_id=?,tmt_pns=?,status=? WHERE id=?");
            $stmt->execute([$nip,$nama,$jabatan,$gol,$satker,$tmt,$status,$id]);
            echo json_encode(['success'=>true,'message'=>'Data pegawai berhasil diupdate']);
        } else {
            $stmt = $db->prepare("INSERT INTO pegawai (nip,nama,jabatan,golongan,satker_id,tmt_pns,status) VALUES (?,?,?,?,?,?,?)");
            $stmt->execute([$nip,$nama,$jabatan,$gol,$satker,$tmt,$status]);
...
if ($module === 'kgb') {
    ...
    if ($action === 'save') {
        ...
        $pegawai = $db->prepare("SELECT id FROM pegawai WHERE nip=?");
        $pegawai->execute([$nip]);
        $peg = $pegawai->fetch();
        if (!$peg) { echo json_encode(['success'=>false,'message'=>'NIP tidak ditemukan']); exit; }

        if ($id) {
            $stmt = $db->prepare("UPDATE kenaikan_gaji SET gol_lama=?,gol_baru=?,tmt=?,no_sk=? WHERE id=?");
            $stmt->execute([$gol_l,$gol_b,$tmt,$no_sk,$id]);
            echo json_encode(['success'=>true,'message'=>'KGB berhasil diupdate']);
        } else {
            $stmt = $db->prepare("INSERT INTO kenaikan_gaji (pegawai_id,gol_lama,gol_baru,tmt,no_sk) VALUES (?,?,?,?,?)");
            $stmt->execute([$peg['id'],$gol_l,$gol_b,$tmt,$no_sk]);
            echo json_encode(['success'=>true,'message'=>'KGB berhasil ditambahkan']);
        }
        exit;
    }
    if ($action === 'delete') {
        $stmt = $db->prepare("DELETE FROM kenaikan_gaji WHERE id=?");
        $stmt->execute([$_GET['id']]);
...
if ($module === 'tunjangan') {
    ...
    if ($action === 'save') {
        ...
        if ($id) {
            ...
            echo json_encode(['success'=>true,'message'=>'Tunjangan berhasil diupdate']);
        } else {
            $stmt = $db->prepare("INSERT INTO tunjangan (pegawai_id,jenis_tunjangan,nominal,periode,status) VALUES (?,?,?,?,?)");
            $stmt->execute([$peg['id'],$jenis,$nominal,$periode,$status]);
            echo json_encode(['success'=>true,'message'=>'Tunjangan berhasil ditambahkan']);
        }
        exit;
    }
    if ($action === 'delete') {
        $stmt = $db->prepare("DELETE FROM tunjangan WHERE id=?");
        $stmt->execute([$_GET['id']]);
...
if ($module === 'slks') {
    ...
    if ($action === 'save') {
        ...
        if ($id) {
            ...
            echo json_encode(['success'=>true,'message'=>'SLKS berhasil diupdate']);
        } else {
            $stmt = $db->prepare("INSERT INTO slks (pegawai_id,periode,nilai_skp,perilaku,total,keterangan) VALUES (?,?,?,?,?,?)");
            $stmt->execute([$peg['id'],$periode,$skp,$perilaku,$total,$ket]);
            echo json_encode(['success'=>true,'message'=>'SLKS berhasil ditambahkan']);
        }
        exit;
    }
    if ($action === 'delete') {
        $stmt = $db->prepare("DELETE FROM slks WHERE id=?");
        $stmt->execute([$_GET['id']]);
...
if ($module === 'arsip') {
    ...
    if ($action === 'upload') {
        ...
        if ($file['size'] > $maxBytes) {
            echo json_encode(['success'=>false,'message'=>'File terlalu besar (maks '.UPLOAD_MAX_MB.'MB)']); exit;
        }
        $ext      = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
        $allowed  = ['pdf','xlsx','xls','docx','doc','png','jpg','jpeg'];
        if (!in_array($ext, $allowed)) {
            echo json_encode(['success'=>false,'message'=>'Tipe file tidak diizinkan']); exit;
        }
    ...
    if ($action === 'delete') {
        ...
        $stmt->execute([$_GET['id']]);
        $row = $stmt->fetch();
        if ($row && file_exists('../' . $row['path_file'])) {
